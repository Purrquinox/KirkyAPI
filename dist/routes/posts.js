...
import { MessageSchema, ErrorSchema, PostSchema, PostWithPublishedSchema, AuthResponses, AuthNotFoundResponses, AuthForbiddenNotFoundResponses, } from "../lib/schemas";
const security = [{ bearerAuth: [] }];
export const postsRouter = new Elysia({ prefix: "/posts" })
    .use(authPlugin)
    // Static segments must come before /:id
    .get("/bookmarks", async ({ userId, query }) => {
    ...
    return { posts: bookmarks.map((b) => transformPost(b.post)), total };
}, {
