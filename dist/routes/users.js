...
import { publicProfileSelect, privateUserSelect, profileSummarySelect, buildPostSelect, transformPost, } from "../lib/select";
import { authPlugin } from "../lib/auth";
import { notify } from "../lib/notifications";
import { NotificationType } from "../../generated/prisma";
import { PublicProfileSchema, ProfileSummarySchema, PrivateUserSchema, UpdatedProfileSchema, PostSchema, MessageSchema, ErrorSchema, AuthResponses, AuthNotFoundResponses, } from "../lib/schemas";
const security = [{ bearerAuth: [] }];
const followCountSelect = {
    _count: { select: { followedBy: true, following: true } },
};
function withCounts(data, _count) {
...
export const usersRouter = new Elysia({ prefix: "/users" })
    ...
    body: t.Object({
        username: t.Optional(t.String({ minLength: 3, maxLength: 20, pattern: "^[a-zA-Z0-9_-]{3,20}$" })),
    ...
    if (pinnedPostId) {
        ...
        if (pinnedRaw)
            pinnedPost = transformPost(pinnedRaw);
    }
    return {
        profile: {
    ...
    if (!existing) {
        set.status = 404;
        return { error: "Not blocking this user" };
    }
    ...
    .get("/:username/posts", async ({ userId, params, query, set }) => {
