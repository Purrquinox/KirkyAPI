...
- **Autocomplete:** Use `GET /search/hashtags?q=<prefix>` to power hashtag autocomplete as the user types `#`.
- **User search:** When building a mention autocomplete (`@username`), use `GET /search/users?q=<query>` and show `username` as the primary result label.
- **Relationship state:** Search results intentionally omit accurate `isFollowing`/`isBlocked` state to keep responses fast. When the user selects a result and you navigate to a profile page, call `GET /users/:username` to get the correct relationship context.
